export default [
    {
        pergunta: 'Normalmente, quantos litros de sangue uma pessoa tem? Em média, quantos são retirados numa doação de sangue?',
        alternativaA: '2 a 4l, retirados 450ml',
        alternativaB: '4 a 6l, retirados 450ml',
        alternativaC: '10l, retirados 2l',
        alternativaD: '7l, retirados 1,5l',
        resposta: 'b'
    },
    {
        pergunta: 'De quem é a famosa frase “Penso, logo existo”?',
        alternativaA: 'Stalin',
        alternativaB: 'Hitler',
        alternativaC: 'Descartes',
        alternativaD: 'Sócrates',
        resposta: 'c'
    },
    {
        pergunta: 'De onde é a invenção do chuveiro elétrico?',
        alternativaA: 'Alemanha',
        alternativaB: 'Rússia',
        alternativaC: 'Brasil',
        alternativaD: 'Inglaterra',
        resposta: 'c'
    },
    {
        pergunta: 'Quais o menor e o maior país do mundo?',
        alternativaA: 'Vaticano e Rússia',
        alternativaB: 'Nauru e China',
        alternativaC: 'Mônaco e Canadá',
        alternativaD: 'Malta e EUA',
        resposta: 'a'
    },
    {
        pergunta: 'Qual o livro mais vendido no mundo a seguir à Bíblia?',
        alternativaA: 'O Senhor dos Anéis',
        alternativaB: 'Dom Quixote',
        alternativaC: 'O Pequeno Príncipe',
        alternativaD: 'Ela, a Feiticeira',
        resposta: 'b'
    },
    {
        pergunta: 'Quantas casas decimais tem o número pi?',
        alternativaA: 'Centenas',
        alternativaB: 'Vinte',
        alternativaC: 'Milhares',
        alternativaD: 'Infinitas',
        resposta: 'd'
    },
    {
        pergunta: 'Atualmente, quantos elementos químicos a tabela periódica possui?',
        alternativaA: '113',
        alternativaB: '109',
        alternativaC: '108',
        alternativaD: '118',
        resposta: 'd'
    },
    {
        pergunta: 'Quanto tempo a luz do Sol demora para chegar à Terra?',
        alternativaA: '12 minutos',
        alternativaB: '1 dia',
        alternativaC: '12 horas',
        alternativaD: '8 minutos',
        resposta: 'd'
    },
    {
        pergunta: 'Qual a nacionalidade de Che Guevara?',
        alternativaA: 'Argentina',
        alternativaB: 'Peruana',
        alternativaC: 'Panamenha',
        alternativaD: 'Cubana',
        resposta: 'a'
    },
    {
        pergunta: 'Em que período da pré-história o fogo foi descoberto?',
        alternativaA: 'Neolítico',
        alternativaB: 'Paleolítico',
        alternativaC: 'Idade dos Metais',
        alternativaD: 'Período da Pedra Polida',
        resposta: 'b'
    },
    {
        pergunta: 'Qual a velocidade da luz?',
        alternativaA: '300 000 000 m/s',
        alternativaB: '150 000 000 m/s',
        alternativaC: '199 792 458 m/s',
        alternativaD: '299 792 458 m/s',
        resposta: 'd'
    },
    {
        pergunta: 'Quem é o autor de “O Príncipe”?',
        alternativaA: 'Maquiavel',
        alternativaB: 'Montesquieu',
        alternativaC: 'Thomas Hobbes',
        alternativaD: 'Rousseau',
        resposta: 'a'
    },
    {
        pergunta: 'Qual destes países é transcontinental?',
        alternativaA: 'Rússia',
        alternativaB: 'Filipinas',
        alternativaC: 'Istambul',
        alternativaD: 'Groenlândia',
        resposta: 'a'
    },
    {
        pergunta: 'Qual foi o recurso utilizado inicialmente pelo homem para explicar a origem das coisas?',
        alternativaA: 'A Filosofia',
        alternativaB: 'A Biologia',
        alternativaC: 'A Matemática',
        alternativaD: 'A Mitologia',
        resposta: 'd'
    },
    {
        pergunta: 'Júpiter e Plutão são os correlatos romanos de quais deuses gregos?',
        alternativaA: 'Ares e Hermes',
        alternativaB: 'Cronos e Apolo',
        alternativaC: 'Zeus e Hades',
        alternativaD: 'Dionísio e Deméter',
        resposta: 'c'
    },
    {
        pergunta: 'Qual o maior animal terrestre?',
        alternativaA: 'Dinossauro',
        alternativaB: 'Elefante africano',
        alternativaC: 'Girafa',
        alternativaD: 'Baleia Azul',
        resposta: 'b'
    },
    {
        pergunta: 'Que líder mundial ficou conhecida como “Dama de Ferro”?',
        alternativaA: 'Dilma Rousseff',
        alternativaB: 'Angela Merkel',
        alternativaC: 'Margaret Thatcher',
        alternativaD: 'Hillary Clinton',
        resposta: 'c'
    },
    {
        pergunta: 'Qual o nome do cientista que descobriu o processo de pasteurização e a vacina contra a raiva?',
        alternativaA: 'Marie Curie',
        alternativaB: 'Blaise Pascal',
        alternativaC: 'Louis Pasteurs',
        alternativaD: 'Antoine Lavoisier',
        resposta: 'c'
    },
    {
        pergunta: 'O que é Brexit?',
        alternativaA: 'Saída do Reino Unido da Zona Euro',
        alternativaB: 'Saída do Reino Unido da União Europeia',
        alternativaC: 'Saída da Inglaterra do Reino Unido',
        alternativaD: 'Fim da monarquia no Reino Unido',
        resposta: 'b'
    },
    {
        pergunta: 'Durante quantos anos Fidel Castro, um dos governantes que esteve mais tempo no poder, esteve à frente de Cuba?',
        alternativaA: '39 anos',
        alternativaB: '32 anos',
        alternativaC: '49 anos',
        alternativaD: '46 anos',
        resposta: 'c'
    },
];
